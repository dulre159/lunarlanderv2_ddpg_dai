...
from main_utils import load_last_run_replay_memory
...
if __name__ == '__main__':
    ...
    eval_and_display = True
    if eval_and_display is True and load_run is False and load_models_from_disk is False:
        sys.exit("Aborting... To make evaluation you must first load a run and at least its models...")
    ...
    rewards_dict = {"avg_10_ep": 0, "avg_50_ep": 0, "avg_100_ep": 0, "avg_tot_ep": 0, "now": 0, "max": 0, "variance": 0,
                    "std": 0}
    ...
    if exp_exp_strategy_name == "just_gnoise":
        ...
        strategy_params += \
            "apnInitialStddev" + str(apnInitialStddev).replace(".", "p") + "_" \
                                                                           "apnDesiredActionStddev" + str(
                apnDesiredActionStddev).replace(".", "p") + "_" + \
    ...
    tot_ep_reward_history = [] if not load_last_run_from_disk or last_run_information is None else last_run_information[
        3]
    tot_eval_ep_avg_reward_history = [] if not load_last_run_from_disk or last_run_information is None else \
        last_run_information[4]
    ...
    if eval_and_display is False:
        ...
        for ep in range(ep, n_eps):
            ...
            # Save step
            if ep % save_ep == 0:
                save_everything(ddpgAgent, last_run_replay_memory_filename, last_run_filename, ep, n_eps, rewards_dict,
                                tot_ep_reward_history, tot_eval_ep_avg_reward_history)
            ...
            # Evaluation step
            if ep % eval_ep == 0:
                do_evaluation(ddpgAgent, n_eval_eps, env, rewards_dict, max_eval_ep_steps,
            ...
            while not done:
                ...
                if done:
                    print(
                        "Ep-N: {}, Ep-Steps: {}, Tot-Ep-R: {:.5f}, Avg-R-100-EP: {:.5f},Avg-R-10-EP: {:.5f}, Avg-R-{}-EP: {:.5f}, Max-R:{:.5f}, EPS:{:.5f}, APN-Scalar:{:.5f}".format(
                            ep, ep_steps, total_reward_per_ep, rewards_dict["avg_100_ep"], rewards_dict["avg_10_ep"],
                            ep,
                            rewards_dict["avg_tot_ep"], rewards_dict["max"], ddpgAgent.eps,
                            ddpgAgent.apnNoise.get_current_stddev()))
        ...
        for eval_mod_ep in range(0, eval_mod_nep):
            total_reward_per_ep = 0
            observation = env.reset()
            done = False
            ep_steps = 0
            ...
            while not done:
                action = ddpgAgent.get_eval_mod_action(observation)
                next_observation, reward, done, info = env.step(action)
                observation = next_observation
                total_reward_per_ep += reward
                env.render()
                if done:
                    print(
                        "Ep-N: {}, Ep-Steps: {}, Tot-Ep-R: {:.5f}".format(
                            ep, ep_steps, total_reward_per_ep))
